...
const forgotPasswordFunctions = {
	sendEmail: async (req, res) => {
		...
		const resetLink = `${protocol}://${host}/user/LinkToVerifyToken?token=${resetToken}`;
		const mailOptions = {
		...
		try {
			...
			const userSelected = await User.findOne({
				$or: [
					...
					{ tokenVerify: token, resetTokenExpiration: { $gt: Date.now() } }
				]
			});
			...
			const isVerificationToken = userSelected.tokenVerify === token;
			...
			if (isVerificationToken) {
				...
				res.redirect("https://site-kong.netlify.app/forget-password?step=3&token=" + token);
			}
		} catch (error) {
